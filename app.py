...
スライドの「中央コンテンツエリアのみ」の画像を作成してください。

【絶対に含めてはいけない要素】
- ヘッダー：上部の帯・タイトルバー・会社名・ロゴ・スライドタイトル欄
- フッター：下部の帯・ページ番号・日付・免責事項欄
- スライドの外枠・罫線・余白装飾

生成するのはスライドテンプレートの「中央の白いコンテンツ部分」だけです。
ヘッダーとフッターはテンプレート側で補完されるため、この画像には一切描かないでください。

【デザイン条件】
- 背景：必ず白色
- 色使い：コーポレートカラーの青色とモノクロのみ
- スタイル：シンプルで読みやすく、コーポレートブランドに沿ったデザイン

以下の内容をもとに画像を生成してください：
{edited_prompt.strip()}

※再確認：ヘッダー・フッターは含めない。中央コンテンツのみを描画すること。"""
